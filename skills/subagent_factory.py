...
from skills.models import SkillDefinition, SubAgentState
...
class SubAgentFactory:
    ...
    def build(self, skill: SkillDefinition, arguments: str):
        ...
        if skill.allowed_tools is not None:
            tools = [self._all_tools_by_name[n] for n in skill.allowed_tools
        ...
        async def sub_executor(state: SubAgentState):
            ...
            has_tools = hasattr(response, "tool_calls") and bool(response.tool_calls) and bool(tools)
            return {
                "messages": [response],
                "turn_count": turn,
