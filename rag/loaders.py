...
def extract_text(path: str | Path, original_filename: str | None = None) -> str:
    ...
    if ext in TABLE_EXTENSIONS:
        return _extract_delimited_text(file_path, delimiter="\t" if ext == ".tsv" else ",")
...
def _extract_pdf_text(path: Path) -> str:
    try:
        ...
        raise TextExtractionError("PDF upload support requires the 'pypdf' package.") from exc
...
def _extract_docx_text(path: Path) -> str:
    try:
        ...
        raise TextExtractionError("DOCX upload support requires the 'python-docx' package.") from exc
...
def _extract_xlsx_text(path: Path) -> str:
    try:
        ...
        raise TextExtractionError("XLSX upload support requires the 'openpyxl' package.") from exc
...
def _extract_pptx_text(path: Path) -> str:
    try:
        ...
        raise TextExtractionError("PPTX upload support requires the 'python-pptx' package.") from exc
