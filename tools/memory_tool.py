...
def memory(
    ...
    if action == "save":
        ...
        mem_key = key.strip() if key and key.strip() else hashlib.sha256(content.encode()).hexdigest()[:12]
    ...
    # ── SEARCH (semantic) ───────────────────────────────────────────────────
        ...
        lines = []
        for r in results:
            score = f" (score: {r.score:.3f})" if hasattr(r, "score") and r.score is not None else ""
